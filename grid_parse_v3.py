...
def save_grid_ui_compatible(grid_data, output_path):
    ...
    if height > 0 and width > 0:
        ...
        img = Image.new("RGB", (new_w * cell_size, new_h * cell_size), (255, 255, 255))
        draw = ImageDraw.Draw(img)
        try:
            font = ImageFont.truetype("Arial.ttf", int(cell_size * 0.35))
        except Exception:
            try:
                font = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", int(cell_size * 0.35))
            except Exception:
                font = ImageFont.load_default()
        for y in range(new_h):
            for x in range(new_w):
                if 1 <= x < new_w - 1 and 1 <= y < new_h - 1:
                    code = grid_2d[y - 1][x - 1]
                    rgb  = beads.get(code, (255, 255, 255)) if beads else (255, 255, 255)
                else:
                    rgb = (200, 200, 200)
                ...
                if 1 <= x < new_w - 1 and 1 <= y < new_h - 1:
                    ...
                    draw.text((x0 + cell_size // 2, y0 + cell_size // 2),
                              str(grid_2d[y - 1][x - 1])[:4],
...
class ColorClusterDialog(QDialog):
    ...
    def _on_save(self):
        ...
        # 如果没有预设路径，使用用户自定义目录或弹窗选择
        if not out_path:
            start_dir = self._output_dir or os.path.expanduser("~")
            chosen = QFileDialog.getSaveFileName(
                self, "保存图纸 JSON", start_dir,
                "JSON 文件 (*_grid.json)"
            )
            ...
            out_path = chosen[0]
            # 确保后缀
            if not out_path.endswith("_grid.json"):
                out_path = out_path.rsplit(".", 1)[0] + "_grid.json"
        elif self._output_dir and os.path.isdir(self._output_dir):
            # 用户设置了自定义目录，但路径还是旧的（图片目录），更新为新目录
            base = os.path.basename(out_path)
            out_path = os.path.join(self._output_dir, base)

        try:
            # 统一使用 save_grid_ui_compatible 保存兼容格式
...
class GridParseWindow(QMainWindow):
    ...
    def _on_generate(self):
        ...
        try:
            ...
            # 保存与 pindou_UI 兼容的图纸（2D列表 + _processed.png）
            ...
            preview_path = os.path.join(out_dir, f"{base_name}_processed.png")
            ...
            # 保存为纯二维列表 JSON（pindou_UI 直接 json.load 读取此格式）
            # 默认编码 + ensure_ascii=True，与 pindou_UI.exe 兼容
            with open(json_path, "w") as f:
                json.dump(grid_2d, f, ensure_ascii=True, indent=2)

            # 保存 _processed.png 预览图（pindou_UI 按此文件名加载）
