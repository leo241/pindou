...
#         draw.line([0, line_y, width * cell, line_y], fill=color, width=width_line)
#
#     # 垂直线（每列）
...
#         width_line = 2 if is_5_line else 1
#         draw.line([line_x, 0, line_x, height * cell], fill=color, width=width_line)
#
#     # --- 第三步：写入色号 ---
#     for y in range(height):
#         for x in range(width):
...
def remove_grid(input_path, output_path, cell=10, border=1):
    img = cv2.imread(input_path)
    h, w = img.shape[:2]
    ny = h // cell
    nx = w // cell
    clean = np.zeros((ny, nx, 3), dtype=np.uint8)
    for y in range(ny):
        for x in range(nx):
            y0 = y*cell + border
            y1 = (y+1)*cell - border
            ...
            # 使用median抗网格线干扰
            clean[y, x] = np.median(pixels, axis=0)
    result = cv2.resize(clean, (w, h), interpolation=cv2.INTER_NEAREST)
    cv2.imwrite(output_path, result)


def save_grid_to_json(grid, filename):
    """
    将 grid 保存到指定的 JSON 文件中。
